...
// initalize the  data ready chip select and reset pins:
    ...
    delay(10);
    WREG(CONFIG1,0xB0+Sample_Rate,BOARD_ADS); // tell on-board ADS to output its clk, set the data rate to 250SPS
    delay(40);
    resetADS(DAISY_ADS); // software reset daisy module if present
    delay(10);
    daisyPresent = smellDaisy(); // check to see if daisy module is present
    if(!daisyPresent){
      WREG(CONFIG1,0x90+Sample_Rate,BOARD_ADS); // turn off clk output if no daisy present   
...
void OpenBCI_32_Daisy::attachDaisy(void){
  WREG(CONFIG1,0xB0+Sample_Rate,BOARD_ADS); // tell on-board ADS to output the clk, set the data rate to 250SPS
  delay(40);
  resetADS(DAISY_ADS); // software reset daisy module if present
  delay(10);
...
// channel settings: powerDown, gain, inputType, SRB2, SRB1 +x++2+  +0++0++5++1++1++0++X+
...
{
   WREG(CONFIG1,0x90+Sample_Rate,BOARD_ADS);
  sampleCounter = 0;
  firstDataPacket = true;
...
{
 csHigh(SD_SS);  // release spi// ??    OBCI.  

 byte *    ptr_buf  =   &circ_buf[(circ_buf_mask & write_ix)*32]  ;

	while(IC4CON & 0x0008) { time_stamp = IC4BUF ;	ere++ ; } ;				// Timer 3 value is in Input Capture 4 Buffer ; ere++ to note fifo depth 
  ...
  csHigh(BOARD_ADS);   					     								//  close SPI
      ...
      memcpy( boardChannelDataRaw, ptr_buf-24 , 24 ) ;  // if(!writeToSDonly) Serial0.write(ptr_buf-24   /* boardChannelDataRaw */  ,24); 
 ...
 // To add : read & push auxliary data
  ...
  if(daisyPresent)
  {
    ptr_buf       = &circ_buf[(circ_buf_mask & write_ix)*32]  ; write_ix++;    	// Hope that the compiler will replace *32 by <<5
...
void OpenBCI_32_Daisy::updateBoardData(){ 
  ...
  if (daisyPresent) for(int i=0; i<8; i++) lastBoardChannelDataInt[i] = boardChannelDataInt[i];  										// if (daisyPresent && !firstDataPacket)  remember the last samples ; memcpy( lastBoardChannelDataInt, boardChannelDataInt,4*8);
...
byte *    ptr_buf = &circ_buf[(circ_buf_mask & read_ix)*32] ;	
  ...
  for(int i = 0; i<8; i++)   { 
     ...
     if(bitRead(boardChannelDataInt[i],23) == 1)  boardChannelDataInt[i] |= 0xFF000000;  else{ boardChannelDataInt[i] &= 0x00FFFFFF; } 	// Convert 24bit to 32bit
  }
...
void OpenBCI_32_Daisy::updateDaisyData(){
  ...
  byte * ptr_buf ;      //byte inByte;
  int byteCounter = 0;
    
  for(int i=0; i<8; i++)  lastDaisyChannelDataInt[i] = daisyChannelDataInt[i];            // if(!firstDataPacket)  remember the last samples// shift and average the byte arrays

  ptr_buf = &circ_buf[(circ_buf_mask & read_ix)*32] ; read_ix++;	 
  
  for(int i = 0; i<8; i++)   { for(int j=0; j<3; j++) daisyChannelDataInt[i] = (daisyChannelDataInt[i]<<8) | *ptr_buf++;    
                               if(bitRead(daisyChannelDataInt[i],23) == 1)  daisyChannelDataInt[i] |= 0xFF000000;  else{ daisyChannelDataInt[i] &= 0x00FFFFFF; }      } 

  // if(daisyPresent && !firstDataPacket){
    byteCounter = 0;
    for(int i=0; i<8; i++){         meanDaisyChannelDataInt[i] = (lastDaisyChannelDataInt[i] + daisyChannelDataInt[i])/2;    }   // average this sample with the last sample
    for(int i=0; i<8; i++){  
      for(int b=2; b>=0; b--){   meanDaisyDataRaw[byteCounter] = (meanDaisyChannelDataInt[i] >> (b*8)) & 0xFF;  byteCounter++;      }    }   // place the average values in the meanRaw array
  //}  
...
{ 
  ...
  if(daisyPresent)//CHECK SAMPLE ODD-EVEN AND SEND THE APPROPRIATE ADS DATA
    if(sampleCounter % 2 != 0) 	Serial0.write(   meanBoardDataRaw,24);    //  for (int i=0; i<24; i++)  Serial0.write(   meanBoardDataRaw[i]);    // send board data on odd  samples
	else                        Serial0.write(   meanDaisyDataRaw,24);    //  for (int i=0; i<24; i++)  Serial0.write(   meanDaisyDataRaw[i]);    // send daisy data on even samples  
  else                          Serial0.write(boardChannelDataRaw,24);    //  for (int i=0; i<24; i++)  Serial0.write(boardChannelDataRaw[i]);     
}
...
void OpenBCI_32_Daisy::RDATA(int targetSS) {          //  use in Stop Read Continuous mode when DRDY goes low
  byte inByte;            //  to read in one sample of the channels
    ...
    for(int i=0; i<3; i++){   //  read in the status register and new channel data
      inByte = xfer(0x00);
      // yj*** boardStat = (boardStat<<8) | inByte; //  read status register (1100 + LOFF_STATP + LOFF_STATN + GPIO[7:4])
    }   
