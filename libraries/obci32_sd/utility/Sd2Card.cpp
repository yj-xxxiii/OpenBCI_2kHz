...
uint8_t Sd2Card::waitNotBusy(uint16_t timeoutMillis) {
  uint16_t t0 = millis();
  do {    if (spiRec() == 0XFF) return true;  }  while (((uint16_t)millis() - t0) < timeoutMillis);
...
/** Write one data block in a multiple block write sequence */ 
uint8_t Sd2Card::writeData(const uint8_t* src ,                        OpenBCI_32_Daisy*   OBCI                    ) 
{
  // wait for previous write to finish

       if ( !waitNotBusy(SD_WRITE_TIMEOUT,      OBCI ) ) {   error(SD_CARD_ERROR_WRITE_MULTIPLE); chipSelectHigh(); return false; }
...
uint8_t Sd2Card::waitNotBusy(uint16_t timeoutMillis,                        OpenBCI_32_Daisy*  OBCI                      ) 
{
  ...
  do 
  {
    ...
    while(!( IC4CON & 0x0008)){} ;              //	if( !(digitalRead(ADS_DRDY)) )	
	{    										//  l'interieur du block dure 14 tmr4 = 14*256*25e-9*1000000 = 89.6 uS //(*OBCI).time_stamp = (long)TMR4 ; 
		chipSelectHigh();  						// broche CS hight
			(*OBCI).ere |=0x10  ;          		//
			(*OBCI).push() ;  					//      
		chipSelectLow();						// broche CS low
	} ; 										
	
     if (spiRec() == 0XFF) return true; // ----Now we have a litle bit of time to write on the SD Card  ------- >>>>>>>>>>>>
 }
